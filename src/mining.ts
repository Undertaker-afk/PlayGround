...
export function buildMining(scene: Scene, world: WorldRefs, veg: VegRefs, inv: InventoryRefs): MiningRefs {
  ...
  function actAtScreenPoint(sx: number, sy: number): boolean {
    ...
    cooldown = COOLDOWN;
    // Generous node hits: taps landing on the ground right next to a vein hit the node
    // (camera micro-drift + small crystals make pixel-exact hits finicky, esp. on touch)
    let nearNode: OreNode | null = null;
    let nearD = 2.4;
    for (const n of nodes) {
      if (!n.alive) continue;
      const d = Math.hypot(n.x - p.x, n.z - p.z);
      if (d < nearD) { nearD = d; nearNode = n; }
    }
    if (nearNode) {
      hitOreNode(nearNode);
      lastAct = { ok: true, kind: 'ore', x: p.x, z: p.z };
  ...
  /** Resolve chop → fill/dig for an already-validated ground point (crosshair fallback). */
  function actAtPoint(x: number, z: number): boolean {
    if (cooldown > 0) { lastAct = { ok: false, kind: 'cooldown' }; return false; }
    cooldown = COOLDOWN;
    const chop = veg.chopTreeAt(x, z, 3.2);
    if (chop.wood > 0) {
      inv.add('wood', chop.wood);
      inv.toast(chop.felled ? `🪵 Tree felled! +${chop.wood} wood` : `🪓 +${chop.wood} wood`);
      lastAct = { ok: true, kind: 'chop', x, z };
      return true;
    }
    if (inv.mode === 'build') { fillAt(x, z); lastAct = { ok: true, kind: 'fill', x, z }; }
    else { digAt(x, z); lastAct = { ok: true, kind: 'dig', x, z }; }
    return true;
  }

  function actAtCrosshair(): boolean {
    if (cooldown > 0) return false;
    // 1) precise pick (ore nodes need exact hits)
    const eng = scene.getEngine();
    const kx = eng.getRenderWidth() / Math.max(1, canvas.clientWidth);
    const ky = eng.getRenderHeight() / Math.max(1, canvas.clientHeight);
    const pick = scene.pick((canvas.clientWidth / 2) * kx, (canvas.clientHeight / 2) * ky,
      (m) => m.metadata?.orePart === true || m.metadata?.isGround === true);
    if (pick?.hit && pick.pickedPoint && Vector3.Distance(pick.pickedPoint, scene.activeCamera!.position) <= REACH) {
      return actAtScreenPoint(canvas.clientWidth / 2, canvas.clientHeight / 2);
    }
    // 2) fallback: march the camera ray to find ground in front of the player
    // (the crosshair sits on the avatar, so raw picks often overshoot past it)
    const cam = scene.activeCamera!;
    const origin = cam.position.clone();
    const target: Vector3 = (cam as any).getTarget
      ? (cam as any).getTarget()
      : origin.add(new Vector3(0, -0.3, -1));
    const dir = target.subtract(origin);
    dir.normalize();
    for (let t = 2.5; t <= REACH; t += 0.5) {
      const px = origin.x + dir.x * t;
      const pz = origin.z + dir.z * t;
      if (Math.abs(px) > 185 || Math.abs(pz) > 185) continue;
      const py = origin.y + dir.y * t;
      if (py <= groundHeight(px, pz) + 0.4) {
        return actAtPoint(px, pz);
      }
    }
    inv.toast('⛏ Aim at the ground');
    lastAct = { ok: false, kind: 'far' };
    return false;
  }
