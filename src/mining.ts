...
import {
  ...
  ParticleSystem, DynamicTexture, Color4, PointerEventTypes
} from '@babylonjs/core';
import {
...
export function buildMining(scene: Scene, world: WorldRefs, veg: VegRefs, inv: InventoryRefs): MiningRefs {
  ...
  for (const vein of ORE_VEINS.slice(0, 12)) {
    ...
    for (let k = 0; k < 3; k++) {
      ...
      cry.material = cryMat;
      cry.parent = root;
      cry.isPickable = false;
      parts.push(cry);
    }
    root.metadata = { oreNode: true };
    root.isPickable = true;
    try { world.shadowGen.addShadowCaster(rock); } catch { /* noop */ }
    nodes.push({ root, parts, x: vein.x, z: vein.z, hp: 3, alive: true, type: vein.type });
  }
  ...
  function actAtScreenPoint(sx: number, sy: number): boolean {
    ...
    const pick = scene.pick(sx, sy, (m) => m.metadata?.oreNode === true || m.metadata?.isGround === true);
    if (!pick?.hit || !pick.pickedPoint) return false;
    const p = pick.pickedPoint;
    if (pick.pickedMesh?.metadata?.oreNode) {
      const node = nodes.find((n) => n.alive && n.root === pick.pickedMesh);
      if (node) {
        if (Vector3.Distance(p, scene.activeCamera!.position) > REACH + 4) { inv.toast('⛏ Too far away'); return false; }
        cooldown = COOLDOWN;
        hitOreNode(node);
        return true;
      }
    }
    if (Vector3.Distance(p, scene.activeCamera!.position) > REACH) { inv.toast('⛏ Too far away'); return false; }
    cooldown = COOLDOWN;
    ...
    if (inv.mode === 'build') fillAt(p.x, p.z);
    else digAt(p.x, p.z);
    return true;
  }
  ...
  function actAtCrosshair(): boolean {
    ...
    return actAtScreenPoint(eng.getRenderWidth() / 2, eng.getRenderHeight() / 2);
  }
  ...
  // Keep node crystals bobbing + face camera glint
  ...
  // Silence unused-part warning (parts kept for future break-apart anim)
  void scene;
