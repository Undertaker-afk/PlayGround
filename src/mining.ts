...
export function buildMining(scene: Scene, world: WorldRefs, veg: VegRefs, inv: InventoryRefs): MiningRefs {
  ...
  function actAtScreenPoint(sx: number, sy: number): boolean {
    ...
    if ((window as any).__pickDebug) {
      console.log(`[pickdbg] in=(${sx},${sy}) scaled=(${Math.round(sx * kx)},${Math.round(sy * ky)}) hit=${pick?.hit} mesh=${pick?.pickedMesh?.name} dist=${pick?.distance?.toFixed(2)}`);
    }
