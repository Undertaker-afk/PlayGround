import {
  ...
  MeshBuilder, Quaternion, Matrix, InstancedMesh
} from '@babylonjs/core';
...
function windify(mat: CustomMaterial, strength = 0.14, freq = 2.2) {
  ...
  mat.Vertex_Before_PositionUpdated(`
    #ifdef INSTANCES
      float phW = world0[3][0]*0.35 + world0[3][2]*0.41;
    #else
      float phW = 0.0;
    #endif
    float hW = clamp(position.y * 1.2, 0.0, 1.5);
    position.x += sin(time*${freq} + phW) * ${strength} * hW;
    position.z += cos(time*${freq * 0.77} + phW*1.3) * ${strength * 0.7} * hW;
    vWindH = hW;
  `);
  return mat;
}
...
export function buildVegetation(scene: Scene, colliders: { x: number; z: number; r: number }[], shadowGen: any, onProgress: (p: number, l: string) => void): VegRefs {
  ...
  {
    console.log('[veg-debug] oakCrown created, totalVerts=', oakCrown.getTotalVertices(), 'hasPositions=', !!oakCrown.getVerticesData('positions'));
    const p = oakCrown.getVerticesData('positions')!;
    console.log('[veg-debug] p is', p ? `array len=${p.length}` : String(p));
    for (let i = 0; i < p.length; i += 3) {
      const j = 1 + (rng() - 0.5) * 0.35;
      p[i] *= j; p[i + 1] *= j * 0.9; p[i + 2] *= j;
    }
    oakCrown.updateVerticesData('positions', p);
  ...
  {
    const p = bushBase.getVerticesData('positions')!;
    for (let i = 0; i < p.length; i += 3) {
      const j = 1 + (rng() - 0.5) * 0.5;
      p[i] *= j; p[i + 1] *= j * 0.7; p[i + 2] *= j;
    }
    bushBase.updateVerticesData('positions', p);
  ...
  {
    const p = rockBase.getVerticesData('positions')!;
    for (let i = 0; i < p.length; i += 3) {
      p[i] *= 1 + (rng() - 0.5) * 0.55;
      p[i + 1] *= 0.6 + (rng() - 0.5) * 0.4;
      p[i + 2] *= 1 + (rng() - 0.5) * 0.55;
    }
    rockBase.updateVerticesData('positions', p);
