...
export function buildVegetation(scene: Scene, quality: QualitySettings, addShadowCaster: (m: Mesh) => void): Vegetation {
  ...
  for (const p of grassPts) {
    ...
    // manual compose with Y rotation (row-major 4x4: scale + rotY + translate)
  ...
  for (const p of treePts) {
    ...
    // keep clearing at spawn (southern meadow)
    if (Math.hypot(p.x, p.z - 85) < 15) continue;
  ...
  for (const p of bushPts) {
    ...
    if (Math.hypot(p.x, p.z - 85) < 9) continue;
  ...
  for (const p of rockPts) {
    ...
    if (Math.hypot(p.x, p.z - 85) < 12) continue; // spawn clearing
  ...
  reedMat.diffuseColor = new Color3(0.35, 0.58, 0.3);
  reedMat.emissiveColor = new Color3(0.08, 0.14, 0.06);
