...
// ---------------------------------------------------------------------------
// Wind plugin: vertex sway for grass (works with instancing + shadows)
// ---------------------------------------------------------------------------
class WindPlugin extends MaterialPluginBase {
  private _time = 0;
  constructor(material: unknown) {
    super(material as never, 'Wind', 200, { WIND: false });
  }
  public setTime(t: number): void {
    this._time = t;
  }
  public override getUniforms() {
    return { ubo: [{ name: 'uTime', size: 1, type: 'float' as never }] };
  }
  ...
  public override getCustomCode(shaderType: string) {
    if (shaderType === 'vertex') {
      return {
        CUSTOM_VERTEX_DEFINITIONS: 'uniform float uTime;',
        // `position` is object-space; uv.y ~0 at base, ~1 at tip for our blades
        CUSTOM_VERTEX_MAIN_END: `
          #ifdef WIND
            float swayPhase = 0.0;
            #ifdef INSTANCES
              swayPhase = float(gl_InstanceID) * 1.7;
            #endif
            float swayAmt = sin(uTime * 2.2 + position.x * 2.0 + position.z * 1.6 + swayPhase) * 0.12 * uv.y;
...
function makeBlade(scene: Scene, width: number, height: number, lean: number): Mesh {
  const blade = MeshBuilder.CreatePlane('blade', { width, height, subdivisions: 1 }, scene);
...
export function buildVegetation(scene: Scene, quality: QualitySettings, addShadowCaster: (m: Mesh) => void): Vegetation {
  ...
  try {
    ...
    (grassMat as any).pluginManager?.setDefines?.('WIND', true);
  } catch { /* no-op */ }
  ...
  function pineMaster(): { trunk: Mesh; leaves: Mesh } {
    ...
    const c1 = MeshBuilder.CreateCone('pc1', { height: 2.6, diameter: 3.2, tessellation: 8 }, scene);
    c1.position.y = 3.6;
    const c2 = MeshBuilder.CreateCone('pc2', { height: 2.2, diameter: 2.5, tessellation: 8 }, scene);
    c2.position.y = 5.0;
    const c3 = MeshBuilder.CreateCone('pc3', { height: 1.8, diameter: 1.7, tessellation: 8 }, scene);
    c3.position.y = 6.2;
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const pm = (grassMat as any).pluginManager;
    const p = pm?.getPlugin?.('Wind') as WindPlugin | undefined;
    if (p) windPlugins.push(p);
  } catch { /* ignore */ }
